...
namespace DGLabGameController
{
	public class ModuleInfo
	{
		...
		public ModuleBase? ModuleInstance { get; set; } // 模块实例
	}
	...
	public partial class FuncSelectPage : UserControl
	{
		...
		private static void StartModule(ModuleInfo info)
		{
			if (Application.Current.MainWindow is MainWindow mw && info.ModuleInstance != null)
			{
				...
				if (apiVersion != AppConfig.ApiVersion)
				{
					new MessageDialog("不兼容的模块", $"此模块的 API 版本：{apiVersion}\r主程序的 API 版本：{AppConfig.ApiVersion}\r是否继续启动？这可能出现兼容性问题！", "继续", data =>
					{
		...
		{
			// 打开模块目录
			if (!Directory.Exists(AppConfig.ModulesPath)) Directory.CreateDirectory(AppConfig.ModulesPath);
			new MessageDialog("程序即将关闭", "添加或卸载模块需在程序关闭状态下进行，是否继续操作？","继续", data =>
			{
				// 打开文件夹
				System.Diagnostics.Process.Start("explorer.exe", AppConfig.ModulesPath);
