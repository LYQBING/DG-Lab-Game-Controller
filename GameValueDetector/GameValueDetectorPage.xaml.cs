...
namespace GameValueDetector
{
    ...
    {
        public static int SleepTime { get; set; } = 200;
        public static int PenaltyValue { get; set; } = 30;

		private GameMonitorConfig? _config;
        ...
        private async void ToggleCheckButton_Click(object sender, RoutedEventArgs e)
        {
			...
			if (process == null)
			{
				...
				{
					Process? match = ProcessManager.FindProcessByName(processes, _config.ProcessName);
					if (match == null)
					{
            ...
            try
            {
                var monitorService = new GameMonitorService(_config, _lastValues);
        ...
        private void JsonFileListBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            try
            {
                ...
                if (_config == null) DebugHub.Warning("脚本加载失败", "无法对其完成解析，无效的 JSON 脚本");
            }
            catch (Exception ex)
            {
